...
function Dashboard({history}) {
	...
	const projectData = [
		...
		{ name: "Customer Database", value: 60, label: "60%", bgColor: "bg-primary" },
		{ name: "Payout Setup", value: 80, label: '80%', bgColor: "bg-info" },
		{ name: "Account Setup", value: 100, label: "Complete", bgColor: "bg-success	" },
	]
						...
						<div className="row">
							{
								smallCardData.map( data => (
									<div className="col-xs-12 col-md-6 mb-3" key={data.name}>
										<ExtraSmallCard name={data.name} value={data.value} bgColor={data.bgColor} />
									</div>
								))
							}
					...
					<div className="col-xs-12 col-md-6 mb-4">
						{
							largeCard2Data.map( data => (
									<LargeCard2 key={data.id} img={ data.img } paragraph={ data.paragraph } link={ data.link } />
							))
						}
					</div>
