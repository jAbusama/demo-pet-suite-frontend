...
function Rooms({history}) {
	...
	const tableData = () => {
		if(state.search.legth > 3) {
			return gState.rooms.filter(bookings => JSON.stringify(bookings).search(new RegExp(state.search, 'ig')) > -1)
		}
		return gState.rooms
	}
				...
				<div className="row">
					...
					<div className="col input-group d-flex justify-content-end">
						<button className='btn btn-primary btn-sm' onClick={showForm}>
							<i className="fas fa-plus-circle mr-1"></i>Add Room
						</button>
					</div>
				</div>
			</div>
