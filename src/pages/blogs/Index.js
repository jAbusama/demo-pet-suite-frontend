...
function Blogs({ history }) {
  ...
  const tableData = () => {
    if (state.search.legth > 3) {
      return gState.blogs.filter(
        (bookings) =>
          JSON.stringify(bookings).search(new RegExp(state.search, 'ig')) > -1
      );
    }
    return gState.blogs;
  };
  ...
  const bookingOption = (
    <div className='btn-option'>
      ...
      <div className='dropdown-menu' aria-labelledby='navbarDropdown'>
        ...
        <form>
          <button className='dropdown-item options btn-sm' type='submit'>
            Delete
          </button>
        </form>
      </div>
    ...
    <MainLayout history={history}>
      ...
      <div className='records'>
        <div className='record-header'>
          ...
          <span className='record-title'>Bookings</span>{' '}
          <span className='record-subtitle small'>Manage All Booking</span>
        </div>
