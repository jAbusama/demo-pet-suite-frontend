...
function Users({ history }) {
  ...
  const tableData = () => {
    if (state.search.legth > 3) {
      return gState.users.filter(
        (user) =>
          JSON.stringify(user).search(new RegExp(state.search, 'ig')) > -1
      );
    }
    return gState.users;
  };
  ...
  const userOption = (id) => (
    <div className='btn-option'>
      ...
      <div className='dropdown-menu' aria-labelledby='navbarDropdown'>
        ...
        <button
          className='dropdown-item options btn-sm'
          onClick={() => gActions.deleteUser(id)}
        >
          Delete
        </button>
      </div>
  ...
  return (
    <MainLayout history={history}>
      {state.showCreate ? (
        <div
          className={`${
            state.showCreate && state.formCreated ? 'd-block' : 'd-none'
          }`}
        >
      ...
      <div className='records'>
        <div className='record-header'>
          ...
          <span className='record-title'>User</span>{' '}
          <span className='record-subtitle small'>Manage All User</span>
        </div>
